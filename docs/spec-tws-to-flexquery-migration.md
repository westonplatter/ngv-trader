...
- **FlexQuery query configuration** _(resolved)_: Verified against `scripts/data/trades.csv` — all six required fields are present in the configured query: `ibExecID` (col 47), `transactionID` (col 45), `orderReference` (col 49), `ibCommission` (col 34), `openCloseIndicator` (col 38), `conid` (col 6). No reconfiguration needed before the sync service is built.
