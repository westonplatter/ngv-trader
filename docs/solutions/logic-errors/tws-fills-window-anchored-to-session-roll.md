...
- _Write path._ `run_intraday_sync` calls `_write_fills` then `_purge_settled` inside one `Session`, with a single `commit()` after both (`:608-617`). `_purge_settled` (`:552`) deletes every `live_executions` row whose `ib_exec_id` matches a `trade_executions` row, plus BAG/combo rows matched by identity via `_settled_combo_summaries` (`:514`) — same account, same instant, both BAG — carrying any trade-group link to the settled execution first. An already-settled fill is written and deleted in the same transaction; it is never visible to a reader.
- _Read path._ `_unsettled_live_executions` in `src/api/routers/trades.py:883` independently filters `LiveExecution.ib_exec_id.not_in(select(TradeExecution.ib_exec_id))`.
...
**One caveat, documented rather than fixed.** The read-path dedup is plain `ib_exec_id` equality, so it does not cover combo **BAG summary** rows, whose live and settled ids come from different families and can never match. `_purge_settled` handles this at write time; the read path does not. Widening the window exposes more rows to that asymmetry.
