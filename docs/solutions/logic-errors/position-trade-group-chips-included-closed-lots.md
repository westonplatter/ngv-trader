...
**Interaction worth knowing:** a synthetic `trade_executions` row (`ib_exec_id` prefixed `SYNTH-`, see [the missing-execution plan](../../plans/2026-07-14-001-fix-missing-opening-execution-plan.md)) exists in prod to back an open futures position that had no real opening fill. It participates in the FIFO walk exactly like a real fill — that is what makes that position attributable at all. Deleting a synthetic opener without a real fill to replace it will silently drop the position into the `_groups_for` fallback.
