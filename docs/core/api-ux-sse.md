...
| -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| **API endpoints** (create/rerun job, archive job, cancel order, trade-group assign/unassign) | Direct call to `broadcaster.publish()` in the same process, after `db.commit()`                                                                       | Immediately after the mutation                 |
...
One multiplexed endpoint serves all topics. The browser opens a single `EventSource` connection per tab. FastAPI's built-in SSE support (`fastapi.sse.EventSourceResponse`) sends a keepalive ping every 15s and sets `Cache-Control: no-cache` and `X-Accel-Buffering: no` on the response itself, so no extra reverse-proxy header configuration is needed to keep the stream unbuffered.
...
| `job.created`       | `jobs`          | `POST /api/v1/jobs/{id}/rerun` (re-enqueue a job)                    | `src/api/routers/jobs.py`                                                                  | `JobResponse`         |
