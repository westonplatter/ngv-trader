...
This writes a `README.md` into `docs/` and every docs subdirectory that contains `.md` files (GitHub auto-renders these). `docs/core/README.md` is generated by `scripts/gen_core_index_html.py` instead. Index files are generated artifacts — never edit them by hand. Front matter (`topics`, `description`) is optional; docs without it are still listed.
