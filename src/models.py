...
# Discovery status values for ActivatedProduct.discovery_status
PRODUCT_DISCOVERY_PENDING = "pending"
PRODUCT_DISCOVERY_ACTIVE = "active"
PRODUCT_DISCOVERY_NEEDS_DISAMBIGUATION = "needs_disambiguation"
PRODUCT_DISCOVERY_UNKNOWN_SYMBOL = "unknown_symbol"


class ActivatedProduct(Base):
    """A futures product we actively maintain in the security master.

    An operator declares a product by symbol alone; the exchange and contract
    metadata are discovered from IBKR (see services.contract_sync) and persisted
    here. A sync job keeps the next ``months_ahead`` calendar months of this
    product's FUT contracts fresh in the ``contracts`` table.
    """

    __tablename__ = "activated_products"
    __table_args__ = (UniqueConstraint("symbol", "sec_type", name="uq_activated_products_symbol_sec_type"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    symbol: Mapped[str] = mapped_column(String, nullable=False)
    sec_type: Mapped[str] = mapped_column(String, nullable=False, default="FUT")
    currency: Mapped[str] = mapped_column(String, nullable=False, default="USD")
    months_ahead: Mapped[int] = mapped_column(Integer, nullable=False, default=12)
    # Discovered from IBKR; NULL until discovery succeeds.
    exchange: Mapped[str | None] = mapped_column(String, nullable=True)
    valid_exchanges: Mapped[str | None] = mapped_column(String, nullable=True)
    multiplier: Mapped[str | None] = mapped_column(String, nullable=True)
    trading_class: Mapped[str | None] = mapped_column(String, nullable=True)
    long_name: Mapped[str | None] = mapped_column(String, nullable=True)
    min_tick: Mapped[float | None] = mapped_column(Float, nullable=True)
    discovery_status: Mapped[str] = mapped_column(String, nullable=False, default=PRODUCT_DISCOVERY_PENDING)
